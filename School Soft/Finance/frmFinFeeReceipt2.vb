...
Public Class frmFinFeeReceipt2
    ...
    Private Sub txtSearchStudName_TextChanged(sender As Object, e As EventArgs) Handles txtSearchStudName.TextChanged
        ...
        Try
            ...
            Me.cmdFeeReceipt.Connection = conn
            Me.cmdFeeReceipt.CommandType = CommandType.Text
            Me.cmdFeeReceipt.CommandText = "SELECT admNo,FullName FROM vwStudClass WHERE (className=@className) AND (stream=@stream) AND (year=@year) AND (FullName LIKE @FullName or AdmNo LIKE @fullName)"
            Me.cmdFeeReceipt.Parameters.Clear()
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        ...
        If Me.chbxDeductLastYearBalance.Checked Then
            If ((CDbl(Me.txtPrevYearBal.Text.Trim) - CDbl(Me.txtPayAmount.Text.Trim)) < 0) And Me.chbxDeductLastYearBalance.Checked Then
            ...
            End If
        Else
            If CDbl(Me.txtPayAmount.Text.Trim) - apportionTotal > 0 Then
                MsgBox("Apportion the fees before saving!", MsgBoxStyle.Exclamation + MsgBoxStyle.ApplicationModal + MsgBoxStyle.OkOnly, "Error Detected")
                Exit Sub
            End If
        ...
        Try
            ...
            Else
                Me.cmdFeeReceipt.Parameters.AddWithValue("@lastYearBalPaid", 0)
            End If
    ...
    Private Sub txtPayAmount_TextChanged(sender As Object, e As EventArgs) Handles txtPayAmount.TextChanged
        ...
        ElseIf Me.txtPayAmount.Text.Trim.Length <= 0 Then
            Exit Sub
        End If
        Dim termOverPayment As Double = 0

        If chbxDeductLastYearBalance.Checked Then
            termOverPayment = CDbl(Me.txtPayAmount.Text.Trim) - CDbl(Me.txtPrevYearBal.Text.Trim) - CDbl(Me.txtTermBal.Text.Trim)
        Else
            termOverPayment = CDbl(Me.txtPayAmount.Text.Trim) - CDbl(Me.txtTermBal.Text.Trim)
        End If
    ...
    Private Sub chbxDeductLastYearBalance_CheckedChanged(sender As Object, e As EventArgs) Handles chbxDeductLastYearBalance.CheckedChanged
